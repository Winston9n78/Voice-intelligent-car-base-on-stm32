...
uint8 ucRegVal;
uint8 start_flag,move_forward_flag,move_back_flag,turn_left_falg,stop_flag;
///用户修改  修改指令顺序：1.到ld3320.h文件中改宏定义名 2.在本文件中修改拼音，和对应好指令名称
...
static uint8 RunASR(void)
{
	...
	for (i=0; i<5; i++)		//防止由于硬件原因导致LD3320芯片工作不正常，所以一共尝试5次启动ASR识别流程
	{
		...
		if (LD_AsrAddFixed()==0)	//添加关键词语到LD3320芯片中
		{
			...
			//检验寄存器初始状态
//      printf("初始状态：\n");
//			printf("1.LD_ReadReg(0x06):%d  \\0\n",LD_ReadReg(0x06));
//			printf("2.LD_ReadReg(0x06):%d  \\135\n",LD_ReadReg(0x06));
//			printf("LD_ReadReg(0x35):%d    \\128\n",LD_ReadReg(0x35));
//			printf("LD_ReadReg(0xb3):%d    \\255\n\n\n",LD_ReadReg(0xb3));
...
//			printf("读写状态：\n");
//			printf("LD_ReadReg(0x35):%d  \\51\n",LD_ReadReg(0x35));
//			printf("LD_ReadReg(0x1b):%d  \\85\n",LD_ReadReg(0x1b));
//			printf("LD_ReadReg(0xb3):%d  \\170  \n\n\n",LD_ReadReg(0xb3));
...
uint8 LD_Check_ASRBusyFlag_b2(void)
{
	...
	for (j=0; j<10; j++)
	{
		if (LD_ReadReg(0xb2) == 0x21)//问题：条件不满足
		{
			flag = 1;
			break;
		}
	...
	printf("LD_ReadReg(0xb2) = %d \\\33  flag=%d\n\n ",LD_ReadReg(0xb2),flag); 
